<template>
  <div class="card-filter-bar">
    <div class="left">
      ...
      <el-select v-model="selectedTypes" multiple collapse-tags placeholder="類型篩選" class="type-select" @change="emitChange">
        <el-option v-for="t in typeOptions" :key="t.value" :label="t.label" :value="t.value" />
      </el-select>
    </div>
    <div class="right">
      <el-select v-model="sortKey" class="sort-select" @change="emitChange">
        <el-option label="按最近創建" value="recent" />
        <el-option label="按標題" value="title" />
        <el-option label="按類型" value="type" />
      </el-select>
...
const emit = defineEmits<{
  (e: 'change', payload: { keyword: string; types: number[]; sortKey: 'recent'|'title'|'type'; density: '舒適'|'緊湊'; view: '卡片'|'列表' }): void
}>()
