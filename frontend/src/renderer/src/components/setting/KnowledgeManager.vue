<template>
  <div class="knowledge-manager">
    <div class="header">
      <h4>知識庫</h4>
      <el-button type="primary" size="small" @click="openEditor()">新建知識</el-button>
    </div>

    <el-table :data="items" height="60vh" size="small" v-loading="loading">
      <el-table-column prop="name" label="名稱" width="90" />
      <el-table-column prop="description" label="描述" min-width="150" />
      <el-table-column label="內置" width="80">
        <template #default="{ row }">
          <el-tag size="small" :type="row.built_in ? 'info' : 'success'">{{ row.built_in ? '內置' : '自定義' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="180" align="right">
        <template #default="{ row }">
          <el-button size="small" @click="openEditor(row)">編輯</el-button>
          <el-popconfirm title="刪除該知識？" @confirm="remove(row)">
            <template #reference>
              <el-button size="small" type="danger" plain :disabled="row.built_in">刪除</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
    ...
    <!-- 將抽屜改爲模態對話框，避免抽屜內嵌抽屜 -->
    <el-dialog v-model="editor.visible" :title="editor.editing ? '編輯知識' : '新建知識'" width="50%" append-to-body>
      <el-form label-position="top" :model="editor.form">
        <el-form-item label="名稱"><el-input v-model="editor.form.name" :disabled="editor.editing && editor.form.built_in" /></el-form-item>
        <el-form-item label="描述"><el-input v-model="editor.form.description" type="textarea" :rows="2" /></el-form-item>
        <el-form-item label="內容"><el-input v-model="editor.form.content" type="textarea" :rows="14" /></el-form-item>
      </el-form>
      <template #footer>
...
async function save() {
  try {
    ...
    if (!f?.name || !f.content) { ElMessage.warning('請填寫名稱與內容'); return }
