...
class CardUpdate(BaseModel):
    ...
    json_schema: Optional[Dict[str, Any]] = None
    ai_params: Optional[Dict[str, Any]] = None
    # AI 修改追蹤字段（前端需要清除 needs_confirmation）
...
class CardExportRequest(BaseModel):
    ...
    scope: CardExportScope = Field(default="all", description="導出範圍")
    card_id: Optional[int] = Field(default=None, description="單卡導出時的卡片ID")
    card_type_id: Optional[int] = Field(default=None, description="按類型導出時的卡片類型ID")
    format: CardExportFormat = Field(default="txt", description="導出格式")

    @model_validator(mode="after")
    def validate_scope_fields(self):
        if self.scope == "single" and self.card_id is None:
            raise ValueError("scope=single 時必須提供 card_id")
        if self.scope == "type" and self.card_type_id is None:
            raise ValueError("scope=type 時必須提供 card_type_id")
        return self
